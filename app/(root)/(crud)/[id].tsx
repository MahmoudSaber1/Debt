...
export default function DebtDetails() {
    ...
    const [personData, setPersonData] = React.useState<PersonProps | null>(null);
    const [refreshing, setRefreshing] = React.useState(false);

    const getPersonData = React.useCallback(async () => {
        try {
            ...
            setPersonData(personDataWithPayments);
        } catch (error) {
            ...
            <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} showsVerticalScrollIndicator={false} className="flex-1">
                <View className="flex-1">
                    ...
                    <View className="flex-1 py-4 gap-2" style={{ direction: "rtl" }}>
                        ...
                        <ScrollView className="flex-1 flex-col gap-2">{personData.payments.length > 0 ? personData.payments.map((payment) => <PaymentCard key={payment.id} item={payment} />) : <EmptyState />}</ScrollView>
                    </View>
                </View>
            </ScrollView>
